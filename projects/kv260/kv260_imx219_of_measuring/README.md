...
- IMX219カメラからのリアルタイム画像取得
- ハードウェア実装されたLucas-Kanadeオプティカルフロー計算
- 指定領域内での動きベクトル計測
- リアルタイムトラッキング表示
- 計測データのCSV出力
- 複数解像度・フレームレート対応 (VGA, 720p, 1000fps高速モード)
...
デフォルトで 1000fps 高速撮影モード (640x130) で起動しますが、以下のオプションで動作モードを変更できます：

```
make load
./kv260_imx219_of_measuring.out vga     # VGAモード (640x480, 60fps)
./kv260_imx219_of_measuring.out 720p    # 720pモード (1280x720, 30fps) 
./kv260_imx219_of_measuring.out 1000fps # 高速モード (640x130, 1000fps)
```
