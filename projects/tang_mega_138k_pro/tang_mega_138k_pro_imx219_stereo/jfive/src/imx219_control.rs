...
impl Imx219Control {
    pub fn new() -> Self {
        Self {
            i2c: JellyI2c::<I2cAccessor>::new(I2cAccessor::new().into(), None),
            running: false,
            binning_h: true,
            binning_v: true,
            aoi_x: 0,
    ...
    pub fn i2c_read(&mut self, addr: u16, buf: &mut [u8]) -> Result<(), &'static str> {
        self.i2c.write(IMX219_DEVADR, &addr.to_be_bytes());
        self.i2c.read(IMX219_DEVADR, buf);
        Ok(())
    }
...
//      println!("readback => {:02x}", self.i2c_read_u8(addr)?);
        Ok(())
    }

    pub fn i2c_read_u8(&mut self, addr: u16) -> Result<u8, &'static str> {
...
//      println!("readback => {:04x}", self.i2c_read_u16(addr)?);
        Ok(())
    }

    pub fn i2c_read_u16(&mut self, addr: u16) -> Result<u16, &'static str> {
        let mut buf: [u8; 2] = [0; 2];
        self.i2c_read(addr, &mut buf)?;
        Ok(u16::from_be_bytes(buf))
    }

    pub fn get_model_id(&mut self) -> Result<u16, &'static str> {
...
//      self.check_open()?;
    ...
    pub fn set_pixel_clock(&mut self, freq: f64) -> Result<(), &'static str> {
        self.pll_vt_mpy = if freq <= 91000000.0 { 57 } else { 87 };
        Ok(())
    }

    pub fn get_pixel_clock(&mut self) -> Result<f64, &'static str> {
...
//      self.check_open()?;
        ...
        let gain = libm::pow(10.0, db / 20.0);
...
//      self.check_open()?;
    ...
    pub fn get_digital_gain(&mut self) -> Result<f64, &'static str> {
