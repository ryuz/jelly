...
using namespace jelly::simulator;
...
using Axi4VideoTest = Axi4sVideoPybind11<Vsim_top, std::uint8_t, std::uint8_t, 8, 8, 1, 1>;

// 生成マクロ
PYBIND11_MODULE(video_test, p) {
    pybind11::class_< Axi4VideoTest >(p, "Axi4VideoTest")
            .def(pybind11::init<int, int>())
            .def("set_image_size",        &Axi4VideoTest::SetImageSize)
            .def("run",                   &Axi4VideoTest::Run)
            .def("read_reg",              &Axi4VideoTest::ReadReg)
            .def("get_read_reg_data",     &Axi4VideoTest::GetReadRegData)
            .def("write_reg",             &Axi4VideoTest::WriteReg)
            .def("write_ireg",            &Axi4VideoTest::WriteIReg)
            .def("wait_bus",              &Axi4VideoTest::WaitBus)
            .def("write_stream",          &Axi4VideoTest::WriteStream)
            .def("get_write_stream_size", &Axi4VideoTest::GetWriteStreamSize)
            .def("read_stream",           &Axi4VideoTest::ReadStream)
            .def("get_read_stream_size",  &Axi4VideoTest::GetReadStreamSize)
            .def("write_image",           &Axi4VideoTest::WriteImage)
            .def("read_image",            &Axi4VideoTest::ReadImage)
            ;
}
