...
//      m_contextp->commandArgs(argc, argv);
        ...
        m_mng->AddNode(jsim::ResetNode_Create(&m_top->reset, 100));
        m_mng->AddNode(jsim::ClockNode_Create(&m_top->clk,   1000.0/100.0));
...
int main() {
    ...
    sim->Run(1000);
    sim->WriteReg(REG_IMG_DEMOSAIC_PARAM_PHASE, 2);
