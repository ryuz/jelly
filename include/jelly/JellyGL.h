...
protected:
	...
	const unsigned long	REG_ADDR_CFG_CORE_ADDR_WIDTH    = 0x04*4;
	const unsigned long	REG_ADDR_CFG_BANK_ADDR_WIDTH    = 0x05*4;
	const unsigned long	REG_ADDR_CFG_PARAMS_ADDR_WIDTH  = 0x06*4;
	const unsigned long	REG_ADDR_CFG_BANK_NUM           = 0x07*4;
	const unsigned long	REG_ADDR_CFG_EDGE_NUM           = 0x08*4;
	const unsigned long	REG_ADDR_CFG_POLYGON_NUM        = 0x09*4;
	const unsigned long	REG_ADDR_CFG_SHADER_PARAM_NUM   = 0x0a*4;
	const unsigned long	REG_ADDR_CFG_EDGE_PARAM_WIDTH   = 0x0b*4;
	const unsigned long	REG_ADDR_CFG_SHADER_PARAM_WIDTH = 0x0c*4;
	const unsigned long	REG_ADDR_CFG_REGION_PARAM_WIDTH = 0x0d*4;

	const unsigned long	REG_ADDR_CTL_ENABLE             = 0x20*4;
	const unsigned long	REG_ADDR_CTL_BANK               = 0x21*4;
	const unsigned long	REG_ADDR_PARAM_WIDTH            = 0x22*4;
	const unsigned long	REG_ADDR_PARAM_HEIGHT           = 0x23*4;
	const unsigned long	REG_ADDR_PARAM_CULLING          = 0x24*4;
	const unsigned long	REG_ADDR_PARAMS_BANK            = 0x28*4;
				...
				for ( size_t i = 0; i < m_polygon.size(); ++i ) {
					if ( CheckRegion(m_polygon[i].region, edge_flags) ) {
						...
						if ( !valid || pp.tex_cord[2] > w ) {
							...
							pp.raw_value[0] = rasterizerParam[i][0].GetRawValue();
							pp.raw_value[1] = rasterizerParam[i][1].GetRawValue();
							pp.raw_value[2] = rasterizerParam[i][2].GetRawValue();
							pp.raw_value[3] = rasterizerParam[i][3].GetRawValue();
							pp.raw_value[4] = rasterizerParam[i][4].GetRawValue();
						}
					}
				}
