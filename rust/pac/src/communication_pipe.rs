...
impl<T: MemAccess> JellyCommunicationPipe<T> {
    ...
    unsafe fn set_tx_irq_enable(&self, enable: bool) {
        self.reg_acc
            .write_reg(REG_TX_IRQ_ENABLE, if enable { 1 } else { 0 });
    }

    unsafe fn set_rx_irq_enable(&self, enable: bool) {
        self.reg_acc
            .write_reg(REG_RX_IRQ_ENABLE, if enable { 1 } else { 0 });
    }

    fn wait_tx(&self) {
        while !self.polling_tx() {
            unsafe {
                self.set_tx_irq_enable(true);
            }
            match self.wait_irq {
