...
template <typename T>
void read_data(std::int8_t* mem_ptr, std::uintptr_t addr)
{
    auto offset = (addr & PAGE_MASK);
    auto data   = *(reinterpret_cast<volatile T *>(&mem_ptr[offset]));
...
int main(int argc, char *argv[])
{
    ...
    if ( param != 1 ) {
        ...
        std::cerr << argv[0] << "[option] <addr>" << std::endl;
    ...
    // access
    switch ( word_size ) {
    case 1: read_data<std::uint8_t>(mem_ptr, addr); break;
    case 2: read_data<std::uint16_t>(mem_ptr, addr); break;
    case 4: read_data<std::uint32_t>(mem_ptr, addr); break;
    case 8: read_data<std::uint64_t>(mem_ptr, addr); break;
    }
