...
def main():
    with open('jelly_data_combine_pack2.v', 'w') as f:
        f.write(file_header)
        ...
        for i in range(num1):
            f.write('            \n')
            for j in range(num2):
                f.write('            input   wire    [DATA%d_%d_BITS-1:0]  s%d_data%d,\n' % (i, j, i, j))
            f.write('            input   wire                        s%d_valid,\n' % i)
            f.write('            output  wire                        s%d_ready,\n' % i)

        f.write('            \n')
        for i in range(num1):
            for j in range(num2):
                f.write('            output  wire    [DATA%d_%d_BITS-1:0]  m_data%d_%d,\n' % (i, j, i, j))
        f.write('            output  wire                        m_valid,\n')
        f.write('            input   wire                        m_ready\n')
        ...
        for i in range(num1):
            ...
            f.write('    \n')
            f.write('    wire    [DATA%d_BITS-1:0]  s%d_data;\n' %(i, i))
            f.write('    wire    [DATA%d_BITS-1:0]  m_data%d;\n' %(i, i))
            ...
            f.write('            (\n')
            for j in range(num1):
                f.write('                .in%d            (s%d_data%d),\n' % (j, i, j))
            f.write('                .out            (s%d_data)\n' % (i))
            f.write('            );\n')
            ...
            f.write('            (\n')
            for j in range(num1):
                f.write('                .out%d           (m_data%d_%d),\n' % (j, i, j))
            f.write('                .in             (m_data%d)\n' % (i))
            f.write('            );\n')

        # split_pack
        f.write('    \n')
        f.write('    jelly_data_combine_pack\n')
        f.write('            #(\n')
        f.write('                .NUM            (NUM),\n')
        for i in range(num1):
        ...
        for i in range(num1):
            f.write('                \n')
            f.write('                .s%d_data        (s%d_data),\n' % (i, i))
            f.write('                .s%d_valid       (s%d_valid),\n' % (i, i))
            f.write('                .s%d_ready       (s%d_ready),\n' % (i, i))

        f.write('                \n')
        for i in range(num1):
            f.write('                .m_data%d        (m_data%d),\n' % ( i, i))
        f.write('                .m_valid        (m_valid),\n')
        f.write('                .m_ready        (m_ready)\n')
